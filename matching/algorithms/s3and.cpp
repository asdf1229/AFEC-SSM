...
struct S3ANDSolver::Impl {
    S3ANDStats &stats;

    explicit Impl(S3ANDStats &stats_ref)
...
S3ANDSolver::S3ANDSolver() : impl_(new Impl(stats)) {}

S3ANDSolver::~S3ANDSolver() = default;

bool S3ANDSolver::init(const Graph *query_graph, const Graph *data_graph,
    ui threshold)
{
...
void S3ANDSolver::printStats() const
{
    impl_->printStats();
}
