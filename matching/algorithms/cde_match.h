...
private:
    ...
    public:
        ...
        bool selectBestActiveEdge(const vector<ui> &frontier_candidates, ActiveEdge &best) const
        {
            bool found = false;
            for (ui u : frontier_candidates) {
                if (u >= solver.qn || solver.in_Mq[u] || solver.frontier_pos[u] == -1) {
                    continue;
                }

                vector<ui> anchors;
                collectLiveAnchors(u, anchors);
                if (anchors.empty()) {
                    continue;
                }
                ...
                ActiveEdge edge;
                edge.u = u;
                edge.live_anchor_count = (ui)anchors.size();
                edge.query_degree = (ui)solver.q_neighbors[u].size();

                for (ui anchor : anchors) {
                    edge.anchor = anchor;
                    edge.anchor_support = countAnchorSupport(u, anchor);
                    if (!found || isBetterActiveEdge(edge, best)) {
                        best = edge;
                        found = true;
                    }
                }
        ...
        FrontierState buildFrontierState(const FrontierState *parent_state, int newly_matched_u)
        {
            ...
            Timer t_select;
            ActiveEdge best_edge;
            bool found_edge = selectBestActiveEdge(solver.active_frontier, best_edge);
            assert(found_edge);
            (void)found_edge;
            solver.stats.frontier_select_time += t_select.elapsed();

            Timer t_component;
            collectComponent(best_edge.u, state);
